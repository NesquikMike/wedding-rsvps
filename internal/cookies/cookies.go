...
func GenerateCookie(name, value string, isProd bool) *http.Cookie {
	cookie := &http.Cookie{
		Name:     name,
		Value:    value,
		Path:     "/",
...
func GenerateBlankCookie(name string, isProd bool) *http.Cookie {
	cookie := &http.Cookie{
		Name:     name,
		Value:    "",
		Path:     "/",
...
func Read(req *http.Request, name string) (string, error) {
...
func ReadEncrypted(req *http.Request, name string, secretKey []byte) (string, error) {
	// Read the encrypted value from the cookie as normal.
	encryptedValue, err := Read(req, name)
	if err != nil {
		return "", err
	}
